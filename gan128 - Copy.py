...
class TxtToImgDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		# if torch.Size([1, 128, 128]) == real_image.shape:
		# 	real_image = real_image.repeat(3, 1, 1)
		# if torch.Size([1, 128, 128]) == wrong_image.shape:
		# 	wrong_image = wrong_image.repeat(3, 1, 1)             
...
if __name__ == "__main__":
    ...
    dataset = TxtToImgDataset(img_dir=IMAGES_DIR, labels_file=LABELS_FILE, captions_dir=CAPTIONS_DIR,
                                transform=transforms.Compose([
                                    ...
                                    # transforms.Lambda(lambda x: x.repeat(3, 1, 1) if torch.Size([1, 128, 128])==x.shape else x),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                                ]))
